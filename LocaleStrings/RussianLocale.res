let localeStrings: LocaleStringTypes.localeStrings = {
  ...
  surchargeMsgAmount: (currency, str) => <>
    {React.string(`Дополнительная сумма в размере${CommonUtils.nbsp}`)}
    <strong> {React.string(`${currency} ${str}`)} </strong>
    {React.string({
      `${CommonUtils.nbsp}будет применено к этой транзакции`
    })}
  </>,
  surchargeMsgAmountForCard: (currency, str) => <>
    {React.string(`Сумма доплаты до${CommonUtils.nbsp}`)}
    <strong> {React.string(`${currency} ${str}`)} </strong>
    {React.string(
      `${CommonUtils.nbsp}будет применено к этой транзакции`,
    )}
