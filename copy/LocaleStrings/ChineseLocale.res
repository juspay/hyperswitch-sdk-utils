let localeStrings: LocaleStringTypes.localeStrings = {
  ...
  surchargeMsgAmount: (currency, str) => <>
    {React.string(`此交易将收取${CommonUtils.nbsp}`)}
    <strong> {React.string(`${currency} ${str}`)} </strong>
    {React.string({`${CommonUtils.nbsp}的附加费用`})}
  </>,
  surchargeMsgAmountForCard: (currency, str) => <>
    {React.string(`此交易将收取最高${CommonUtils.nbsp}`)}
    <strong> {React.string(`${currency} ${str}`)} </strong>
    {React.string(`${CommonUtils.nbsp}的附加费用`)}
  </>,
  ...
  pixKeyLabel: `Pix 密钥`,
  cardDetailsLabel: "卡详细信息",
  firstName: "名",
  lastName: "姓",
  billingDetails: "账单详情",
  requiredText: "必需的",
  cardHolderNameRequiredText: "必须填写持卡人姓名",
  lastNameRequiredText: "姓氏必填",
  cardExpiresText: "到期",
  addPaymentMethodLabel: "添加新的付款方式",
  walletDisclaimer: "选择后将保存钱包详细信息",
  deletePaymentMethod: "删除",
  invalidDigitsCardHolderNameError: "持卡人姓名不能包含数字",
  invalidDigitsNickNameError: "昵称不能超过2位数字",
  nickNameLengthExceedError: "昵称不能超过12个字符",
  enterValidCardNumberErrorText: "请输入有效的卡号。",
  ibanEmptyText: "IBAN不能为空",
}
